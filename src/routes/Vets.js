...
const Vets = () => {
  ...
  const searchVet = () => {
    if (searchInput.trim() === "") {
      ...
      const searchedVets = vetsList.filter((vet) =>
  ...
  return (
    <div className="flex flex-col lg:flex-row w-full h-screen">
      ...
      <div className="bg-slate-900 flex flex-col w-5/5 lg:w-4/5 text-white">
        <Header title="Vets" />
        <div className="border rounded mx-auto flex w-[57%] mt-8 lg:mt-0">
          <input
            type="text"
        ...
        <ul className="py-6 grid place-items-center grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-y-8 lg:overflow-y-scroll">
          {filteredVetsList &&
            filteredVetsList.map((vet) => (
              <li
                key={vet.id}
                className="group flex flex-col items-center gap-2 p-4 rounded-lg bg-slate-800 hover:bg-slate-600 duration-300 hover:scale-105 cursor-pointer"
              >
                <img
                  src={vet.vet_photo}
                  alt={vet.name}
                  className="w-full lg:w-48 rounded-[100%] border border-8 border-slate-700 group-hover:border-slate-500"
                />
                      ...
                      if (vet.specialization_id === specialization.id) {
                        return (
                          <p
                            ...
                            className="tracking-wider text-slate-300 group-hover:text-slate-200"
                          >
                            {specialization.name}
                          </p>
                        );
                ...
                <p className="flex items-center gap-2 text-teal-400 group-hover:text-teal-300">
                  <IoIosPricetag />{" "}
                  <span className="flex gap-1">
                    {vet.fees}
                    <span>DH</span>
                  </span>{" "}
                  <span className="text-white text-sm">Per visit</span>
                </p>
                <Link
                  to={`/home/vets/${vet.id}`}
                  className="cursor-pointer p-2 w-full bg-sky-700 group-hover:bg-sky-600 text-center rounded text-lg tracking-wide"
                >
                  vet's details
                </Link>
              </li>
